...
def train_lstm(settings: PredictiveSettings) -> dict:
    params = settings.lstm_params
    train_seq, valid_seq = build_train_valid_windows(settings, params["window_size"])
    device = torch.device("cpu")

    model = LSTMRegressor(input_size=len(settings.features), hidden_size=params["hidden_size"]).to(device)
    ...
    pred_df["model_name"] = "lstm"
    pred_df["y_pred"] = preds
    pred_df["target_policy"] = "rul_capped"
    pred_df.to_parquet(settings.out_dir / "03_valid_predictions_lstm.parquet", index=False)
    torch.save(
        {
        ...
        settings.models_dir / "lstm_model.pt",
    )
    ...
    write_json(settings.out_dir / "03_train_metadata_lstm.json", metadata)
    return metadata
